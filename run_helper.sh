...
echo "run trace"
# run hw trace
python ${ppt_gpu_dir}/scripts/run_hw_trace.py -F ${single_app} -Y ${apps_yaml} -B ${benchmarks} -T ${trace_dir} -D 0 --trace_tool ${ppt_gpu_dir}/tracing_tool/tracer.so
fi

if (( $1 & 4 )); then
echo "run profling"
# run_hw profling
python ${ppt_gpu_dir}/scripts/run_hw_profling.py -F ${single_app} -Y ${apps_yaml} -B ${benchmarks} -T ${trace_dir} --loop_cnt 3
python ${ppt_gpu_dir}/scripts/get_stat_hw.py -F ${single_app} -Y ${apps_yaml} -B ${benchmarks} -T ${trace_dir} -o ${res_hw_json}

# run_hw_ncu
python ${ppt_gpu_dir}/scripts/run_hw_profling.py -F ${single_app} -Y ${apps_yaml} -B ${benchmarks} -T ${trace_dir} --ncu --loop_cnt 3
python ${ppt_gpu_dir}/scripts/get_stat_hw.py -F ${single_app} -Y ${apps_yaml} -B ${benchmarks} -T ${trace_dir} --ncu -o ${res_hw_ncu_json} --loop 3
python ${ppt_gpu_dir}/scripts/draw/convert_cpi_stack.py -i ${res_hw_ncu_json} -I "ncu" -o ${res_hw_cpi_json}
fi
...
echo "run simulation"
# run_sim
python ${ppt_gpu_dir}/scripts/run_simulation.py -F ${single_app} -M "mpiexec -n 2" -Y ${apps_yaml} -B ${benchmarks} -T ${trace_dir} -H TITANV --granularity 2 -R ${single_report_dir}
python ${ppt_gpu_dir}/scripts/get_stat_sim.py -F ${single_app} -Y ${apps_yaml} -B ${benchmarks} -T ${single_report_dir} -o ${res_sim_json}
python ${ppt_gpu_dir}/scripts/draw/convert_cpi_stack.py -i ${res_sim_json} -I "ppt_gpu" -o ${res_sim_cpi_json}
python ${ppt_gpu_dir}/scripts/draw/convert_cpi_stack.py -i ${res_sim_json} -I "ppt_gpu_sched"  -o ${res_sim_sched_cpi_json}
