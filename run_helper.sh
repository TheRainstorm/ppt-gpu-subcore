...
pyrhon ${ppt_gpu_dir}/scripts/run_hw_trace.py -B ${hw_new_benchmarks} -T ${trace_dir} -D 0 --trace_tool ${ppt_gpu_dir}/tracing_tool/tracer.so
}

run_hw(){
# hw run
python ${ppt_gpu_dir}/scripts/run_hw_profling.py -B ${hw_new_benchmarks} -T ${trace_dir} --loop_cnt 3
python ${ppt_gpu_dir}/scripts/get_stat_hw.py -B ${benchmarks} -T ${trace_dir} -o ${res_hw_json}
}

run_hw_ncu(){
# hw run
python ${ppt_gpu_dir}/scripts/run_hw_profling.py -B ${hw_new_benchmarks} -T ${trace_dir} --ncu --loop_cnt 3

python ${ppt_gpu_dir}/scripts/get_stat_hw.py -B ${benchmarks} -T ${trace_dir} --ncu -o ${res_hw_ncu_json} --loop 3
...
cd ${ppt_gpu_dir}
# run sim
python ${ppt_gpu_dir}/scripts/run_simulation.py -M "mpiexec -n 2" -B ${sim_new_benchmarks} -T ${trace_dir} -H TITANV --granularity 2 -R ${report_dir} 2>&1 | tee run_simulation.log

# get stat
python ${ppt_gpu_dir}/scripts/get_stat_sim.py -B ${benchmarks} -T ${report_dir} -o ${res_sim_json}
...
# run_hw_ncu
python ${ppt_gpu_dir}/scripts/run_hw_profling.py -F ${single_app} -B ${benchmarks} -T ${trace_dir} --ncu --loop_cnt 3
python ${ppt_gpu_dir}/scripts/get_stat_hw.py -F ${single_app} -B ${benchmarks} -T ${trace_dir} --ncu -o ${res_hw_ncu_json} --loop 3
python ${ppt_gpu_dir}/scripts/draw/convert_cpi_stack.py -i ${res_hw_ncu_json} -I "ncu" -o ${res_hw_cpi_json}
fi
